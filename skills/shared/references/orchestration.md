...
(`~/.agentic-sdk/<project>/runs/<slug>/decisions.edn`); each entry is one terse
...
- **Plan up front, in a sub-agent.** Dispatch a `planner` that runs
  ...
  full plan to the gitignored `~/.agentic-sdk/<project>/runs/<campaign>/plan.edn` and
...
- **Checkpoint for resume.** Record progress (phases done, in flight,
  ...
  `~/.agentic-sdk/<project>/runs/<campaign>/` so a crash or a token ceiling does not
...
minimal, gitignored checkpoint under `~/.agentic-sdk/<project>/runs/<slug>/` for one
...
Guidance captured mid-task lives in `~/.agentic-sdk/<project>/guidance/inbox.edn`
